...
class MeshViewer(object):
    ...
    def __init__(self, width=1200, height=800,
        ...
        self.ptcld_constructor = trimesh.PointCloud
        self.tripoints_to_pypoints = pyrender.Mesh.from_points
        ...
        self.viewer = pyrender.Viewer(self.scene, use_raymond_lighting=True,
                                      ...
                                      cull_faces=False,
                                      run_in_thread=True,
                                      registered_keys=registered_keys, **kwargs)
    ...
    def create_pointcloud(self, vertices, color=(0.3, 0.3, 0.3, 1.0)):
        from trimesh import transformations

        rot = self.transf(np.radians(180), [1, 0, 0])

        matrix = np.asanyarray(rot,
                               order='C',
                               dtype=np.float64)
        new_vertices = transformations.transform_points(vertices, matrix=matrix)
        mesh = self.tripoints_to_pypoints(new_vertices, colors=color)

        return mesh

    def update_pointcloud(self, vertices):
        if not self.viewer.is_active:
            return
        ...
        for node in self.scene.get_nodes():
            if node.name == 'scene_points':
                self.scene.remove_node(node)
                break

        scene_points = self.create_pointcloud(vertices, color=(1., 1., 1., 1.0))

        self.scene.add(scene_points, name='scene_points')

        self.viewer.render_lock.release()

    def save_screenshot(self, filename=None):
        self.viewer._save_image(filename=filename)
